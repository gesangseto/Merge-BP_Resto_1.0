...
const FormNoteItem = React.forwardRef((props, ref) => {
  const {item, selectedItems, onCancel, onSave, isOpen, isTakeAway} = props;
  const [itemData, setItemData] = useState({});
  const [listOption, setListOption] = useState([]);
  const [selectedList, setSelectedList] = useState({});
  const modalizeNoteItem = useRef(null);
  ...
  useEffect(() => {
    if (isOpen) {
      modalizeNoteItem.current?.open();
    } else {
      modalizeNoteItem.current?.close();
    }
  }, [isOpen]);
